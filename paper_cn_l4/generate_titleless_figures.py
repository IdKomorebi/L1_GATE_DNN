...
def plot_budget_lines_one(csv_path: Path, out_name: str) -> None:
    ...
    for method in methods:
        ...
        ax.plot(
            ...
            linewidth=1.45 if is_key else 0.75,
...
def _choose_topn_csv(default_root: str) -> Path:
    l4_csv = PAPER / "topn_incremental_results_l4.csv"
    if l4_csv.exists():
        return l4_csv
    l4_runs = sorted(
        (ROOT / "NEW_PROJECT/outputs/data2025_Processed_V2/RedundantNoiseExplanation").glob("RedundantNoiseExplanation_l4*/topn_incremental_results.csv"),
...
def plot_topn_noise_one(csv_path: Path, center: str, out_name: str) -> None:
    ...
    if not full_rows.empty:
        ...
        ax.scatter([full["feature_count"]], [full["best_test_r2"]], marker="s", s=28, color="#1f77b4", edgecolor="#d62728", linewidth=0.8, zorder=4)
        ax.annotate(
            f"Full{int(full['feature_count'])}\n{full['best_test_r2']:.4f}",
            xy=(full["feature_count"], full["best_test_r2"]),
            xytext=(-28, 9),
            textcoords="offset points",
            fontsize=6.6,
            color="#d62728",
    ...
    ax.set_ylim(ymin, 1.05)
    ax.set_xlim(0, max(float(sub["feature_count"].max()) + 4, 36))
    common_axes(ax)
    save(fig, out_name)
