...
OUT_CSV = PAPER / "topn_incremental_results_l4.csv"
...
def read_existing() -> pd.DataFrame:
    ...
    merged["_priority"] = merged["output_dir"].astype(str).str.contains("TopNIncremental_L4").astype(int)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Supplement l4 Top-N points without rerunning the whole original experiment.")
    parser.add_argument("--top-ns", nargs="+", type=int, default=[30, 35, 40, 45, 50, 55, 60, 65, 66, 67])
    ...
    script6 = load_module("run_baselines_l4", PROJECT / "scripts" / "06_run_baselines.py")
    script10 = load_module("explain_noise_l4", PROJECT / "scripts" / "10_explain_redundant_noise.py")
    ...
    for center in args.centers:
        ...
        for top_n in args.top_ns:
            ...
            output_dir = BASE_RUN / center_label(center) / "TopNIncremental_L4" / f"DNN_top{int(top_n):02d}"
