"""
基于统计先验知识驱动的 L2 平滑风险映射门控中心变量预测网络。

使用方式（直接在本文件底部改参数后运行）：
    python MetaGating_L2Risk_CenterDataPredictor.py
"""
...
class MetaGatedRegressor(nn.Module):
    ...
    def __init__(self, in_dim: int, correlation_vectors: np.ndarray, dropout: float = 0.05):
        """
        Args:
            in_dim: 输入特征维度
        ...
        dropout = max(0.0, min(float(dropout), 0.5))

        # 后续 MLP：加入轻量 Dropout，降低小样本下后期抖动和过拟合
...
def gated_loss(pred: torch.Tensor, target: torch.Tensor, gates: torch.Tensor, lambda_l2: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    带 L2 平滑正则化的损失函数。

    L1 会更偏向把 gate 压成稀疏选择；L2 对较大的 gate 给出平滑惩罚，
    更适合作为连续风险分数映射。
    ...
    # 使用 Mean 避免特征数变化导致正则项量级漂移
    l2 = torch.mean(gates ** 2)
    return mse + lambda_l2 * l2, mse, l2
...
class TrainConfig:
    ...
    epochs: int = 200
    lr: float = 1.5e-3
    lambda_l2: float = 0.01  # L2 平滑正则化系数；偏风险映射，不再强推稀疏选择
    warmup_epochs: int = 50  # === 核心修改3：预热轮数 ===
    risk_threshold: float = 0.50  # 风险阈值；gate >= 0.5 等价于风险 logit >= 0
    dropout: float = 0.05  # 轻量 Dropout，缓解 MLP 后期过拟合
    net_weight_decay: float = 1e-4  # 只约束 MLP 权重，避免过度记忆训练集
    meta_weight_decay: float = 1e-5  # 极轻约束风险映射层，防止 W/b 后期继续放大
    early_stopping_patience: int = 25  # test_r2 长期不提升则提前停止
    early_stopping_min_delta: float = 1e-5  # 视为有效提升的最小 test_r2 增量
...
def train_model(cfg: TrainConfig) -> None:
    ...
    # 自动构建输出目录结构：MetaGatingL2RiskModelOutput/BasedOnData{i}/CenteredOn{中心变量}(...参数...)/
    ...
    lambda_str = f"{cfg.lambda_l2:.0e}" if cfg.lambda_l2 < 0.01 else f"{cfg.lambda_l2:.4f}".rstrip('0').rstrip('.')
    dropout_str = f"{cfg.dropout:.2f}".rstrip('0').rstrip('.')
    net_wd_str = f"{cfg.net_weight_decay:.0e}" if cfg.net_weight_decay < 0.001 else f"{cfg.net_weight_decay:.4f}".rstrip('0').rstrip('.')
    # 在目录名中包含关键泛化参数，避免覆盖旧结果
    center_dir_name = (
        f"CenteredOn{center_safe}"
        f"(epochs={cfg.epochs},l2={lambda_str},risk={cfg.risk_threshold},"
        f"drop={dropout_str},wd={net_wd_str},pat={cfg.early_stopping_patience})"
    )
    base_dir = os.path.join("MetaGatingL2RiskModelOutput", f"BasedOnData{data_num}", center_dir_name)
    ...
    risk_features_fig_path = os.path.join(base_dir, "risk_features.png")
    ...
    risk_map_path = os.path.join(base_dir, "risk_map.csv")
    ...
    model = MetaGatedRegressor(
        in_dim=len(related),
        correlation_vectors=correlation_vectors,
        dropout=cfg.dropout,
    ).to(DEVICE)
    normalized_correlation_vectors = model.correlation_vectors.detach().cpu().numpy()
    # === L2 风险映射补偿：给 W_meta / b_meta 设置更高学习率，让风险映射层学得更主动 ===
    # 注意：必须将单个参数用列表包裹，否则会被迭代成切片（非叶子节点）
    optimizer = SimpleAdam([
        {'params': [model.W_meta], 'lr': cfg.lr * 6, 'weight_decay': cfg.meta_weight_decay},
        {'params': [model.b_meta], 'lr': cfg.lr * 1.5, 'weight_decay': cfg.meta_weight_decay},
        {'params': model.net.parameters(), 'lr': cfg.lr, 'weight_decay': cfg.net_weight_decay},
    ])

    log_rows = []
    gate_history = []  # 记录每个 epoch 的门控参数
    risk_features_history = []  # 记录每个 epoch 的风险特征数量
    w_meta_history = []  # 记录每个 epoch 的 W_meta 参数 (6个值)
    b_meta_history = []  # 记录每个 epoch 的 b_meta 参数
    # 记录特定 epoch 的风险特征（用于 keydata 分析）
    key_epochs = [10, 20, 50, 100, 200]
    keydata_records = {}  # {epoch: {"risk_indices": [...], "risk_names": [...], "risk_count": int}}
    wb_snapshots = {}  # {epoch: {"W_meta": [...], "b_meta": float}}
    best_state = None
    best_r2 = -1e9
    best_epoch = None
    best_gate_values = None
    best_w_meta_values = None
    best_b_meta_value = None
    no_improve_epochs = 0

    print(f"开始训练... 前 {cfg.warmup_epochs} 轮为 Warm-up (无 L2 惩罚)")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        current_lambda = 0.0 if epoch <= cfg.warmup_epochs else cfg.lambda_l2
        ...
        # 计算风险特征数量：gate 是 0-1 连续风险分数，默认 >= 0.5 视为风险偏高
        risk_mask = gate_values >= cfg.risk_threshold
        risk_count = np.sum(risk_mask)
        risk_features_history.append(risk_count)
        
        # 记录特定 epoch 的风险特征和 W/b 快照
        if epoch in key_epochs:
            risk_indices = np.where(risk_mask)[0].tolist()
            risk_names = [related[i] for i in risk_indices]
            
            # 计算推断风险：W_meta^T * Ri + b_meta > 0 说明存在推断风险
            gate_logits = np.matmul(normalized_correlation_vectors, w_meta_values.reshape(-1, 1)).flatten() + b_meta_value
            ...
            keydata_records[epoch] = {
                ...
                "risk_scores": {related[i]: float(gate_values[i]) for i in range(len(related))},
        ...
        log_rows.append(
            {
                ...
                "risk_features": risk_count,
            }
        )

        # 保存最好模型（按 test_r2），同时基于有效提升做 early stopping
        previous_best_r2 = best_r2
        is_new_best = test_r2 > previous_best_r2
        is_meaningful_improvement = test_r2 > previous_best_r2 + cfg.early_stopping_min_delta
        ...
        if is_new_best:
            ...
            best_state = {
                ...
                "related": related,
                "correlation_vectors": correlation_vectors,
                "normalized_correlation_vectors": normalized_correlation_vectors,
                "regularization": "L2",
                "lambda_l2": cfg.lambda_l2,
                "risk_threshold": cfg.risk_threshold,
                "dropout": cfg.dropout,
                "net_weight_decay": cfg.net_weight_decay,
                "meta_weight_decay": cfg.meta_weight_decay,
            ...
            best_epoch = epoch
            best_gate_values = gate_values.copy()
            best_w_meta_values = w_meta_values.copy()
            best_b_meta_value = b_meta_value

        if epoch <= cfg.warmup_epochs or is_meaningful_improvement:
            no_improve_epochs = 0
        else:
            no_improve_epochs += 1
        ...
        print(
            ...
            f"risk_features={risk_count}/{len(related)} | "
            f"Mean Risk: {gate_values.mean():.3f} | "
            f"NoImprove: {no_improve_epochs}/{cfg.early_stopping_patience}"
        )
        ...
        if (
            ...
            print(
                ...
                f"best_epoch={best_epoch}, best_test_r2={best_r2:.4f}"
            )
            break
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Risk Score / Gate Value (gi)")
    plt.title("Risk Scores Over Epochs (with Warm-up)")
    ...
    # 风险特征数量随 epoch 变化
    plt.figure(figsize=(7, 4))
    plt.plot(log_df["epoch"], log_df["risk_features"], marker='o', linestyle='-')
    plt.xlabel("Epoch")
    plt.ylabel("Number of Risk Features")
    plt.title("Number of Risk Features Over Epochs")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(risk_features_fig_path, dpi=150)
    plt.close()
    ...
    # 保存最佳 epoch 对应的连续风险映射表
    if best_gate_values is not None and best_w_meta_values is not None and best_b_meta_value is not None:
        best_gate_logits = np.matmul(
            normalized_correlation_vectors,
            best_w_meta_values.reshape(-1, 1)
        ).flatten() + best_b_meta_value
        risk_map_df = pd.DataFrame({
            "feature_index": [name_mapping[rel] for rel in related],
            "feature_name": related,
            "risk_score": best_gate_values,
            "risk_logit": best_gate_logits,
            "is_risk": best_gate_values >= cfg.risk_threshold,
            "best_epoch": best_epoch,
        }).sort_values("risk_score", ascending=False)
        risk_map_df.to_csv(risk_map_path, index=False, encoding="utf-8-sig")
        print(f"风险映射表已保存: {risk_map_path}")
    ...
    if best_state is not None:
        ...
        print(f"可视化图表已保存: {loss_fig_path}, {r2_fig_path}, {gate_fig_path}, {risk_features_fig_path}")
        print(f"W_meta 和 b_meta 演化图已保存: {w_meta_fig_path}, {b_meta_fig_path}")
        print(f"名称映射表已保存: {name_mapping_path}")


if __name__ == "__main__":
    # 在这里改参数（输出路径会自动生成：MetaGatingL2RiskModelOutput/BasedOnData{i}/CenteredOn{中心变量}(epochs={},l2={},risk={})/）
    CONFIG = TrainConfig(
        center_rel_path="CenterDataRelationships/center_realdata4_电压节点消耗功率.csv",
        data_path="RealData/data4.csv",
        correlation_path="CenterDataRelationships/center_realdata4_电压节点消耗功率.csv",  # 相关系数文件路径
        batch_size=50,
        epochs=200,
        lr=1.5e-3,
        lambda_l2=0.01,  # L2 平滑正则化系数；可试 0.005 - 0.03
        warmup_epochs=50,  # 预热 50 轮
        risk_threshold=0.50,  # 风险阈值；gate >= 0.5 视为推断风险偏高
        dropout=0.05,
