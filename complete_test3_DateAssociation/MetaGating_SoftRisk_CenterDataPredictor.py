"""
基于统计先验知识驱动的 SoftRisk 平滑风险映射中心变量预测网络。

使用方式（直接在本文件底部改参数后运行）：
    python MetaGating_SoftRisk_CenterDataPredictor.py
"""
...
class MetaGatedRegressor(nn.Module):
    ...
    def __init__(
        ...
        self.gate_scale_strength = float(gate_scale_strength)
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        gates = self.get_gates()  # (in_dim,)
        
        # SoftRisk 不让 gate 直接做 0-1 特征筛选，而是围绕 1 做温和缩放。
        # gate_scale_strength=0.5 时，特征缩放范围约为 [0.75, 1.25]。
        feature_scale = 1.0 + self.gate_scale_strength * (gates - 0.5)
        gated_x = x * feature_scale.unsqueeze(0)  # (batch_size, in_dim)
    ...
    def get_gates(self) -> torch.Tensor:
        """获取当前连续风险分数 gi。"""
        return torch.sigmoid(self.get_gate_logits() / self.gate_temperature)


def gated_loss(pred: torch.Tensor, target: torch.Tensor, model: MetaGatedRegressor, lambda_l2: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    带 L2 平滑正则化的损失函数。

    这里不再惩罚 gate 本身，否则仍会把非核心特征压向 0，效果接近软特征选择。
    改为惩罚风险映射层 W_meta / b_meta 的幅度，让风险分数更平滑、不易两极分化。
    
    Args:
        pred: 预测值
        target: 真实值
        model: SoftRisk 风险映射模型
        lambda_l2: 风险映射层 L2 正则化系数
    ...
    # 平滑风险映射层，而不是直接压低风险分数。
    l2 = torch.mean(model.W_meta ** 2) + 0.1 * torch.mean(model.b_meta ** 2)
...
class TrainConfig:
    ...
    epochs: int = 200
    lr: float = 1e-3
    lambda_l2: float = 0.001  # 风险映射层 L2 系数；比直接惩罚 gate 更平滑
    warmup_epochs: int = 50  # === 核心修改3：预热轮数 ===
    risk_threshold: float = 0.50  # 风险阈值；gate >= 0.5 等价于风险 logit >= 0
    gate_temperature: float = 3.0  # sigmoid 温度，越大风险分数越不容易饱和
    gate_scale_strength: float = 0.5  # 特征缩放强度，0.5 表示缩放约为 [0.75, 1.25]
...
def train_model(cfg: TrainConfig) -> None:
    ...
    # 自动构建输出目录结构：MetaGatingSoftRiskModelOutput/BasedOnData{i}/CenteredOn{中心变量}(...参数...)/
    ...
    # 在目录名中包含 risk_threshold 参数，避免和旧 L1 输出混淆
    temp_str = f"{cfg.gate_temperature:.2f}".rstrip('0').rstrip('.')
    scale_str = f"{cfg.gate_scale_strength:.2f}".rstrip('0').rstrip('.')
    center_dir_name = (
        f"CenteredOn{center_safe}"
        f"(epochs={cfg.epochs},l2={lambda_str},risk={cfg.risk_threshold},temp={temp_str},scale={scale_str})"
    )
    base_dir = os.path.join("MetaGatingSoftRiskModelOutput", f"BasedOnData{data_num}", center_dir_name)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            optimizer.zero_grad()
            pred = model(xb)
            total_loss, mse_loss, l2_loss = gated_loss(pred, yb, model, current_lambda)
            total_loss.backward()
            optimizer.step()
        ...
        if epoch in key_epochs:
            ...
            # 计算推断风险：温度缩放不改变 logit 正负，只缓和 sigmoid 饱和速度。
        ...
        if test_r2 > best_r2:
            ...
            best_state = {
                ...
                "regularization": "SoftRisk-L2-on-risk-layer",
    ...
    if best_gate_values is not None and best_w_meta_values is not None and best_b_meta_value is not None:
        ...
        risk_map_df = pd.DataFrame({
            ...
            "temperature_scaled_logit": best_gate_logits / cfg.gate_temperature,
            "feature_scale": 1.0 + cfg.gate_scale_strength * (best_gate_values - 0.5),
...
if __name__ == "__main__":
    # 在这里改参数（输出路径会自动生成：MetaGatingSoftRiskModelOutput/BasedOnData{i}/CenteredOn{中心变量}(...参数...)/）
    CONFIG = TrainConfig(
        ...
        epochs=200,
        lr=1e-3,
        lambda_l2=0.001,  # 风险映射层 L2；可试 0.0005 - 0.003
        warmup_epochs=50,  # 预热 50 轮
        risk_threshold=0.50,  # 风险阈值；gate >= 0.5 视为推断风险偏高
        gate_temperature=3.0,  # 越大越平滑；可试 2.0 - 5.0
        gate_scale_strength=0.5,  # 越小越不像特征选择；可试 0.3 - 0.7
        train_ratio=0.8,
        random_state=42,
    )
