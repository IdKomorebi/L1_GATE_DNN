...
% 阶段名置于虚线框内的标题栏；子步骤仅保留主名称；第三阶段为一个总阶段
% 独立编译，用于导出高分辨率位图
...
% ============ 阶段一（3.2 节）============
\node[hbar,below=4.0mm of in]  (hA) {阶段一（3.2 节）  显式推断关系识别与去除};
...
% ============ 阶段二（3.3 节）============
\node[hbar,below=4.6mm of a3]  (hB) {阶段二（3.3 节）  隐性推断源定位};
...
% ============ 阶段三（3.4 节）：一个总阶段 ============
\node[hbar,below=4.6mm of b3]  (hC) {阶段三（3.4 节）  处置与残余风险再评估};
