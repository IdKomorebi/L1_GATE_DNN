...
def fig3(d: list[dict], name: str):
    ...
    ax.set_ylabel("测试 R²")
    ax.legend(loc="upper center", ncol=3, fontsize=5.6, framealpha=.92,
              borderaxespad=0.2, handlelength=1.4, columnspacing=1.0)
    fig.tight_layout(pad=0.3)
    fig.savefig(OUT / name); plt.close(fig)


# ---------------- 图 5：综合处置前后（每数据集一张子图） ----------------
def fig5(d: list[dict], name: str, ncand: dict[str, int] | None = None):
    """柱上标注与图 3 保持一致，都写成“选中数/候选数”。

    fig5 的数据项里只有 nsel，候选数从 fig3 的同名目标取——同一份
    figure_data.json，不引入任何外部数据。
    """
    ncand = ncand or {}
    ...
    ax.bar(x + w / 2, [r["after"] for r in d], w, label="综合处置后",
           color="#b23b3b", edgecolor="k", lw=0.4)
    for i, r in enumerate(d):
        lab_i = (f"{r['nsel']}/{ncand[r['name']]}" if r["name"] in ncand
                 else f"{r['nsel']}")
        ax.text(i, max(r["before"], 0) + .014, lab_i,
                ha="center", fontsize=5.4, color="#444")
    ax.axhline(0, color="k", lw=0.7)
    ax.set_xticks(x)
    ax.set_xticklabels([r["name"] for r in d], rotation=36, ha="right", fontsize=6.2)
    ax.margins(y=0.10)
    ax.set_ylim(top=ax.get_ylim()[1] * 1.16)   # 给图例留位置
    ax.grid(axis="y", alpha=.25, ls=":")
    ax.set_ylabel("测试 R²")
    ax.legend(loc="upper right", ncol=2, fontsize=5.8, framealpha=.92,
              borderaxespad=0.2, handlelength=1.4, columnspacing=1.0)
    fig.tight_layout(pad=0.3)
    fig.savefig(OUT / name); plt.close(fig)
...
def fig4(csv: Path, name: str):
    ...
    for m, mm, l, c, lb in zip(order, mk, ls, col, lab):
        ax.plot(x, piv[m], marker=mm, ls=l, color=c, ms=3.2, lw=1.0, label=lb,
                zorder=4 if m == "DGatingDNN" else 2,
                markerfacecolor="none" if m != "DGatingDNN" else c)
    ax.set_xticks(x)
    ax.set_xticklabels(piv.index, rotation=36, ha="right", fontsize=6.2)
    ax.margins(y=0.09)          # 不留边距时 Lasso 在阻塞价上的最低点会被下边缘切掉
    ax.grid(alpha=.25, ls=":")
    ax.set_ylabel("测试 R²")
...
def fig6(name_prefix="fig6"):
    ...
    for k, t in enumerate(tg):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([f"{a}{c}" for a, c in zip(r.条件, r.水平)],
                           rotation=36, ha="right", fontsize=6.2)
        ax.margins(y=0.12)
        ax.grid(alpha=.25, ls=":")
        ax.set_ylabel("测试 R²")
        if k == 0:
            # 左下角会压住块状缺失 30% 那个最低点，放右下角不遮挡数据
            ax.legend(loc="lower right", framealpha=.9, fontsize=6)
        fig.tight_layout(pad=0.3)
        fig.savefig(OUT / f"{name_prefix}{chr(97 + k)}_robustness.png"); plt.close(fig)


if __name__ == "__main__":
    print("生成 paper8 图（子图独立文件，标题由 LaTeX 排版）：")
    ...
    nc_pjm = {r["name"]: r["ncand"] for r in DATA["fig3"]["pjm"]}
    nc_rts = {r["name"]: r["ncand"] for r in DATA["fig3"]["rts"]}
    fig5(DATA["fig5"]["pjm"], "fig5a_mitigation_pjm.png", nc_pjm)
    print("  fig5a_mitigation_pjm.png")
    fig5(DATA["fig5"]["rts"], "fig5b_mitigation_rts.png", nc_rts)
