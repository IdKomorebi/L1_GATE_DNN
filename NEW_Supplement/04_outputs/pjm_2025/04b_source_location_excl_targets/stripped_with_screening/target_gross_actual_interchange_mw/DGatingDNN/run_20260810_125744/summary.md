...
运行时间 20260810_125744　方法 **DGatingDNN**

## 一、这次跑的是什么

- 目标字段：`gross_actual_interchange_mw`（总实际交换功率）
- 数据：pjm_2025，8760 行
- 候选字段：**38 个**（排除其余关注字段；剥离第一类关系（0 个字段）；经过多指标初筛）
- 网络结构：64-32-16，训练 200 轮，批大小 50，学习率 0.001
- 数据划分：随机 80% 训练 / 20% 测试，随机种子 42

## 二、结论先看这里

- 从 38 个候选字段里选出 **16 个**推断源（占 42.1%）
- 只用这 16 个字段重新训练普通 DNN，测试 R² = **0.8768**
- 作为对照，用全部 38 个字段的普通 DNN 测试 R² = 0.8666
- 没被选中的 22 个字段单独训练，测试 R² = 0.5455

> 用 42% 的字段达到了和全量相当的精度（相差 +0.0102），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `gross_inadv_interchange_mw` | 总非计划交换 | 0.1750 |
| 2 | `gen_fuel_multiple_fuels_pct` | 多燃料占比 | 0.1572 |
| 3 | `gen_fuel_nuclear_mw` | 核电出力 | 0.1538 |
| 4 | `gross_sched_interchange_mw` | 总计划交换 | 0.1505 |
| 5 | `gen_fuel_other_renewables_mw` | 其他可再生出力 | 0.1498 |
| 6 | `gen_fuel_gas_pct` | 燃气占比 | 0.1497 |
| 7 | `gen_fuel_coal_pct` | 燃煤占比 | 0.1465 |
| 8 | `da_as_as_req_mw_synchronized_reserve` | 日前同步备用需求 | 0.1424 |
| 9 | `gen_fuel_nuclear_pct` | 核电占比 | 0.1181 |
| 10 | `solar_generation_mw` | 光伏出力 | 0.1136 |
| 11 | `system_energy_price_da` | 日前能量价格 | 0.1133 |
| 12 | `gen_fuel_hydro_mw` | 水电出力 | 0.0959 |
| 13 | `gen_fuel_oil_mw` | 燃油出力 | 0.0956 |
| 14 | `da_as_ss_mw_primary_reserve` | 日前主用备用自调度 | 0.0813 |
| 15 | `total_pjm_assigned_reg` | 调频分配量 | 0.0449 |
| 16 | `total_pjm_reg_purchases` | 调频购买量 | 0.0343 |
...
|---|---|---|---|
| DNN | 0.8666 | 第 118 轮 | 38 |
| L1GateDNN | 0.8762 | 第 188 轮 | 16 |
| DGatingDNN | 0.8782 | 第 200 轮 | 16 |
