...
运行时间 20260811_005803　方法 **DGatingDNN**

## 一、这次跑的是什么

- 目标字段：`total_lmp_da`（日前总电价）
- 数据：pjm_2025，8760 行
- 候选字段：**35 个**（排除其余关注字段；剥离第一类关系（0 个字段）；经过多指标初筛）
- 网络结构：64-32-16，训练 200 轮，批大小 50，学习率 0.001
- 数据划分：随机 80% 训练 / 20% 测试，随机种子 42

## 二、结论先看这里

- 从 35 个候选字段里选出 **1 个**推断源（占 2.9%）
- 只用这 1 个字段重新训练普通 DNN，测试 R² = **0.9865**
- 作为对照，用全部 35 个字段的普通 DNN 测试 R² = 0.9943
- 没被选中的 34 个字段单独训练，测试 R² = 0.9611

> 用 3% 的字段达到了和全量相当的精度（相差 -0.0078），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `system_energy_price_da` | 日前能量价格 | 0.3474 |
...
|---|---|---|---|
| DNN | 0.9943 | 第 187 轮 | 35 |
| L1GateDNN | 0.9913 | 第 196 轮 | 1 |
| DGatingDNN | 0.9885 | 第 10 轮 | 1 |
