...
运行时间 20260810_050352　方法 **DGatingDNN**
...
- 从 48 个候选字段里选出 **2 个**推断源（占 4.2%）
- 只用这 2 个字段重新训练普通 DNN，测试 R² = **1.0000**
- 作为对照，用全部 48 个字段的普通 DNN 测试 R² = 1.0000
- 没被选中的 46 个字段单独训练，测试 R² = 0.7874

> 用 4% 的字段达到了和全量相当的精度（相差 +0.0000），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `da_as_nsr_mw_primary_reserve` | 非同步备用 | 0.0501 |
| 2 | `da_as_as_mw_synchronized_reserve` | 日前同步备用实际量 | 0.0402 |
...
| `total_pjm_loc_credit` | 机会成本信用 | 0.0000 |
...
|---|---|---|---|
| DNN | 1.0000 | 第 183 轮 | 48 |
| L1GateDNN | 1.0000 | 第 199 轮 | 1 |
| DGatingDNN | 1.0000 | 第 147 轮 | 2 |
