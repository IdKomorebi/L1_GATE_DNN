...
运行时间 20260810_035711　方法 **DGatingDNN**
...
- 从 49 个候选字段里选出 **3 个**推断源（占 6.1%）
- 只用这 3 个字段重新训练普通 DNN，测试 R² = **0.9975**
- 作为对照，用全部 49 个字段的普通 DNN 测试 R² = 0.9992
- 没被选中的 46 个字段单独训练，测试 R² = 0.9990

> 用 6% 的字段达到了和全量相当的精度（相差 -0.0017），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `gen_fuel_nuclear_pct` | 核电占比 | 0.0422 |
| 2 | `total_pjm_rt_load_mwh` | 实时负荷电量 | 0.0264 |
| 3 | `gen_fuel_nuclear_mw` | 核电出力 | 0.0241 |
...
| 字段 | 业务含义 | 门控值 |
|---|---|---|
| `gen_fuel_coal_mw` | 燃煤出力 | 0.0067 |
| `gross_sched_interchange_mw` | 总计划交换 | 0.0027 |
| `total_pjm_reg_purchases` | 调频购买量 | 0.0000 |
...
|---|---|---|---|
| DNN | 0.9992 | 第 192 轮 | 49 |
| L1GateDNN | 0.9988 | 第 114 轮 | 1 |
| DGatingDNN | 0.9978 | 第 7 轮 | 3 |
