...
### 第 1 层　[高精度近似]　相对残差 9.02e-03（相当于 R² = 0.9999）

```
total_lmp_da = +1.013*system_energy_price_da +1.014*congestion_price_da -0.3123
```

- 这条关系用到 2 个字段
- **删掉贡献最大的 `system_energy_price_da`（日前能量价格）**
- 删完之后候选池剩 57 个字段，目标仍能被线性回归推到 R² = **0.9175**

## 三、最终从候选池里剥掉了这些字段

| 层 | 字段 | 业务含义 | 剥离后线性 R² |
|---|---|---|---|
| 1 | `system_energy_price_da` | 日前能量价格 | 0.9175 |

候选池：**58 → 43 个字段**（实际剥掉 15 个）。
...
剥离之后目标仍能被推到 R² = 0.9175，**这部分才是隐性推断的部分**。
