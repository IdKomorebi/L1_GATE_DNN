...
运行时间 20260810_040132　方法 **DGatingDNN**
...
- 只用这 1 个字段重新训练普通 DNN，测试 R² = **0.9999**
- 作为对照，用全部 58 个字段的普通 DNN 测试 R² = 0.9999
- 没被选中的 57 个字段单独训练，测试 R² = 0.9999

> 用 2% 的字段达到了和全量相当的精度（相差 -0.0000），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `prelim_load_avg_hourly` | 预估小时负荷 | 0.0665 |
...
|---|---|---|---|
| DNN | 0.9999 | 第 181 轮 | 58 |
| L1GateDNN | 0.9999 | 第 148 轮 | 1 |
| DGatingDNN | 0.9999 | 第 157 轮 | 1 |
