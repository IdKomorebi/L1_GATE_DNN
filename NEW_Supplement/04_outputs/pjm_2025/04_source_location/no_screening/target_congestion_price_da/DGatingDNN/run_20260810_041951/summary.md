...
运行时间 20260810_041951　方法 **DGatingDNN**
...
- 从 58 个候选字段里选出 **3 个**推断源（占 5.2%）
- 只用这 3 个字段重新训练普通 DNN，测试 R² = **0.9999**
- 作为对照，用全部 58 个字段的普通 DNN 测试 R² = 0.9960
- 没被选中的 55 个字段单独训练，测试 R² = 0.6243

> 用 5% 的字段达到了和全量相当的精度（相差 +0.0039），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `total_lmp_da` | 日前总电价 | 0.8384 |
| 2 | `system_energy_price_da` | 日前能量价格 | 0.7835 |
| 3 | `marginal_loss_price_da` | 日前边际损耗价 | 0.0270 |
...
|---|---|---|---|
| DNN | 0.9960 | 第 196 轮 | 58 |
| L1GateDNN | 1.0000 | 第 172 轮 | 2 |
| DGatingDNN | 1.0000 | 第 170 轮 | 3 |
