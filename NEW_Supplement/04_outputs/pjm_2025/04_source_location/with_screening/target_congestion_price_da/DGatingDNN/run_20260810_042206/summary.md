...
运行时间 20260810_042206　方法 **DGatingDNN**
...
- 从 47 个候选字段里选出 **3 个**推断源（占 6.4%）
- 只用这 3 个字段重新训练普通 DNN，测试 R² = **0.9999**
- 作为对照，用全部 47 个字段的普通 DNN 测试 R² = 0.9962
- 没被选中的 44 个字段单独训练，测试 R² = 0.7105

> 用 6% 的字段达到了和全量相当的精度（相差 +0.0037），说明这组字段确实承载了推断目标所需的主要信息。

## 三、选出来的推断源

| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `total_lmp_da` | 日前总电价 | 0.8323 |
| 2 | `system_energy_price_da` | 日前能量价格 | 0.7637 |
| 3 | `marginal_loss_price_da` | 日前边际损耗价 | 0.0275 |
...
|---|---|---|---|
| DNN | 0.9962 | 第 195 轮 | 47 |
| L1GateDNN | 1.0000 | 第 198 轮 | 2 |
| DGatingDNN | 1.0000 | 第 143 轮 | 3 |
