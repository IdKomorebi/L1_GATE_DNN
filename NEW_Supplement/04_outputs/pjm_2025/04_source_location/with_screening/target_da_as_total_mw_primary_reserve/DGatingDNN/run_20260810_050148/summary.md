...
运行时间 20260810_050148　方法 **DGatingDNN**
...
- 从 39 个候选字段里选出 **2 个**推断源（占 5.1%）
- 只用这 2 个字段重新训练普通 DNN，测试 R² = **1.0000**
- 作为对照，用全部 39 个字段的普通 DNN 测试 R² = 1.0000
- 没被选中的 37 个字段单独训练，测试 R² = 0.8221
...
| 排名 | 字段 | 业务含义 | 门控值 |
|---|---|---|---|
| 1 | `da_as_nsr_mw_primary_reserve` | 非同步备用 | 0.0478 |
| 2 | `da_as_as_mw_synchronized_reserve` | 日前同步备用实际量 | 0.0385 |
...
|---|---|---|---|
| DNN | 1.0000 | 第 176 轮 | 39 |
| L1GateDNN | 1.0000 | 第 157 轮 | 1 |
| DGatingDNN | 1.0000 | 第 105 轮 | 2 |
