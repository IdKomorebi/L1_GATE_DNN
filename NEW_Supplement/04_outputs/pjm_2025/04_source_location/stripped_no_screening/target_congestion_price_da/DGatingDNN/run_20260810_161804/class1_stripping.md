...
### 第 1 层　[近似关系]　相对残差 6.54e-02（相当于 R² = 0.9957）

```
congestion_price_da = -0.9934*system_energy_price_da +0.981*total_lmp_da +0.2971
```

- 这条关系用到 2 个字段
- **删掉贡献最大的 `total_lmp_da`（日前总电价）**
- 删完之后候选池剩 57 个字段，目标仍能被线性回归推到 R² = **0.1906**

## 三、最终从候选池里剥掉了这些字段

| 层 | 字段 | 业务含义 | 剥离后线性 R² |
|---|---|---|---|
| 1 | `total_lmp_da` | 日前总电价 | 0.1906 |
...
剥离之后目标仍能被推到 R² = 0.1906，**这部分才是隐性推断的部分**。
