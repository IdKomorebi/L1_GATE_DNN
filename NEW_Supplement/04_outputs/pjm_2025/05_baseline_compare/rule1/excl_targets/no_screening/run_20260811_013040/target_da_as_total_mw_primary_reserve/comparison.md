...
| Lasso | 线性稀疏（1996） | **0.8765** |
| Pearson | 相关系数排序（只看两两关系） | **0.8713** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.8707** |
| DGatingDNN | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.8531** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.8400** |

> 同预算下最好的是 **XGBoost**（R² = 0.8800）。

## 三、不同字段数下的完整曲线

只比一个预算容易碰巧，下表给出各方法在多个字段数下的表现（对应图 `fig_topk_curve.png`）：

| 方法 | k=1 | k=3 | k=8 | k=17 | k=22 | k=34 | k=47 |
|---|---|---|---|---|---|---|---|
| XGBoost | 0.4669 | 0.5569 | 0.8636 | 0.8800 | 0.8716 | 0.8542 | 0.8422 |
| Lasso | 0.4669 | 0.5292 | 0.8427 | 0.8765 | 0.8479 | 0.8400 | 0.8551 |
| Pearson | 0.4669 | 0.4643 | 0.8731 | 0.8713 | 0.8552 | 0.8535 | 0.8468 |
| LassoNet | 0.4669 | 0.6130 | 0.8568 | 0.8707 | 0.8750 | 0.8612 | 0.8516 |
| DGatingDNN | 0.5798 | 0.8143 | 0.8751 | 0.8531 | 0.8696 | 0.8514 | 0.8391 |
| STG | 0.4676 | 0.5451 | 0.8354 | 0.8400 | 0.8468 | 0.8435 | 0.8511 |
...
| 8 | `total_pjm_rt_load_mwh` | 实时负荷电量 |
| 9 | `da_as_ss_mw_primary_reserve` | 日前主用备用自调度 |
| 10 | `gen_fuel_wind_mw` | 风电出力 |
| 11 | `total_pjm_rmpcp_cr` | 调频性能信用额 |
| 12 | `total_pjm_self_sched_reg` | 调频自调度量 |
| 13 | `gen_fuel_solar_pct` | 光伏占比 |
| 14 | `gross_inadv_interchange_mw` | 总非计划交换 |
| 15 | `da_as_as_req_mw_primary_reserve` | 日前主用备用需求 |
