...
- 全部 49 个字段的普通 DNN：R² = 0.9676（能力上限参照）
...
| 方法 | 说明 | 测试 R² |
|---|---|---|
| DGatingDNN ⭐ | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.9698** |
| Lasso | 线性稀疏（1996） | **0.9678** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.9624** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.9623** |
| XGBoost | 树模型特征重要性（2016） | **0.8980** |
| Pearson | 相关系数排序（只看两两关系） | **0.8914** |

> 同预算下最好的是 **DGatingDNN**（R² = 0.9698）。
...
|---|---|---|---|---|---|---|---|
| DGatingDNN | 0.0407 | 0.3476 | 0.9388 | 0.9698 | 0.9702 | 0.9700 | 0.9658 |
| Lasso | 0.0407 | 0.4519 | 0.9466 | 0.9678 | 0.9689 | 0.9674 | 0.9686 |
| LassoNet | 0.2278 | 0.5759 | 0.7987 | 0.9624 | 0.9693 | 0.9681 | 0.9657 |
| STG | 0.0373 | 0.2210 | 0.7812 | 0.9623 | 0.9668 | 0.9628 | 0.9669 |
| XGBoost | 0.2923 | 0.5889 | 0.8233 | 0.8980 | 0.9603 | 0.9703 | 0.9670 |
| Pearson | 0.2278 | 0.5533 | 0.7902 | 0.8914 | 0.8970 | 0.9639 | 0.9624 |
...
| 14 | `gen_fuel_multiple_fuels_pct` | 多燃料占比 |
