...
- 全部 49 个字段的普通 DNN：R² = 0.9263（能力上限参照）
...
| 方法 | 说明 | 测试 R² |
|---|---|---|
| DGatingDNN ⭐ | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.9431** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.9377** |
| XGBoost | 树模型特征重要性（2016） | **0.9360** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.9213** |
| Lasso | 线性稀疏（1996） | **0.9204** |
| Pearson | 相关系数排序（只看两两关系） | **0.9202** |

> 同预算下最好的是 **DGatingDNN**（R² = 0.9431）。

## 三、不同字段数下的完整曲线

只比一个预算容易碰巧，下表给出各方法在多个字段数下的表现（对应图 `fig_topk_curve.png`）：

| 方法 | k=1 | k=3 | k=9 | k=18 | k=23 | k=36 | k=49 |
|---|---|---|---|---|---|---|---|
| DGatingDNN | 0.3356 | 0.7461 | 0.8991 | 0.9431 | 0.9411 | 0.9288 | 0.9346 |
| STG | 0.0755 | 0.4686 | 0.8940 | 0.9377 | 0.9317 | 0.9364 | 0.9344 |
| XGBoost | 0.5375 | 0.7056 | 0.8965 | 0.9360 | 0.9313 | 0.9462 | 0.9319 |
| LassoNet | 0.5137 | 0.6342 | 0.8716 | 0.9213 | 0.9318 | 0.9429 | 0.9375 |
| Lasso | 0.4868 | 0.8787 | 0.9216 | 0.9204 | 0.9270 | 0.9370 | 0.9347 |
| Pearson | 0.5375 | 0.7418 | 0.9282 | 0.9202 | 0.9380 | 0.9335 | 0.9335 |
...
| 7 | `da_as_as_req_mw_primary_reserve` | 日前主用备用需求 |
