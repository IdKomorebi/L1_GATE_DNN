...
- 全部 34 个字段的普通 DNN：R² = 0.8946（能力上限参照）
...
| 方法 | 说明 | 测试 R² |
|---|---|---|
| DGatingDNN ⭐ | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.8954** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.8917** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.8892** |
| Lasso | 线性稀疏（1996） | **0.8797** |
| XGBoost | 树模型特征重要性（2016） | **0.8627** |
| Pearson | 相关系数排序（只看两两关系） | **0.8446** |

> 同预算下最好的是 **DGatingDNN**（R² = 0.8954）。
...
|---|---|---|---|---|---|---|---|
| DGatingDNN | 0.3412 | 0.7332 | 0.8573 | 0.8954 | 0.8922 | 0.8960 | 0.8908 |
| LassoNet | 0.3412 | 0.7610 | 0.8019 | 0.8917 | 0.8927 | 0.8906 | 0.8919 |
| STG | 0.1143 | 0.4695 | 0.7957 | 0.8892 | 0.8896 | 0.8946 | 0.8870 |
| Lasso | 0.1023 | 0.4170 | 0.7410 | 0.8797 | 0.8771 | 0.8906 | 0.8955 |
| XGBoost | 0.2313 | 0.6321 | 0.8132 | 0.8627 | 0.8964 | 0.8910 | 0.8930 |
| Pearson | 0.3412 | 0.7610 | 0.7977 | 0.8446 | 0.8680 | 0.8726 | 0.8875 |
