...
- 全部 42 个字段的普通 DNN：R² = 0.9002（能力上限参照）
...
| 方法 | 说明 | 测试 R² |
|---|---|---|
| DGatingDNN ⭐ | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.9046** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.8964** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.8938** |
| Lasso | 线性稀疏（1996） | **0.8883** |
| XGBoost | 树模型特征重要性（2016） | **0.8800** |
| Pearson | 相关系数排序（只看两两关系） | **0.8569** |

> 同预算下最好的是 **DGatingDNN**（R² = 0.9046）。

## 三、不同字段数下的完整曲线

只比一个预算容易碰巧，下表给出各方法在多个字段数下的表现（对应图 `fig_topk_curve.png`）：

| 方法 | k=1 | k=3 | k=9 | k=18 | k=23 | k=36 | k=42 |
|---|---|---|---|---|---|---|---|
| DGatingDNN | 0.3412 | 0.4615 | 0.8852 | 0.9046 | 0.8962 | 0.8968 | 0.9077 |
| STG | 0.1667 | 0.3066 | 0.7923 | 0.8964 | 0.8935 | 0.9032 | 0.9054 |
| LassoNet | 0.3412 | 0.7610 | 0.8343 | 0.8938 | 0.9006 | 0.9028 | 0.9043 |
| Lasso | 0.1023 | 0.5259 | 0.8372 | 0.8883 | 0.8957 | 0.9006 | 0.8948 |
| XGBoost | 0.2935 | 0.6689 | 0.8301 | 0.8800 | 0.8937 | 0.8988 | 0.9032 |
| Pearson | 0.3412 | 0.6919 | 0.8071 | 0.8569 | 0.8616 | 0.8880 | 0.9025 |
...
| 6 | `da_as_as_req_mw_synchronized_reserve` | 日前同步备用需求 |
| 7 | `net_actual_interchange_mw` | 净实际交换 |
| 8 | `gen_fuel_coal_mw` | 燃煤出力 |
| 9 | `total_pjm_rt_load_mwh` | 实时负荷电量 |
| 10 | `gen_fuel_oil_pct` | 燃油占比 |
| 11 | `gen_fuel_hydro_mw` | 水电出力 |
| 12 | `gen_fuel_coal_pct` | 燃煤占比 |
| 13 | `da_as_as_mw_primary_reserve` | 日前主用备用实际量 |
| 14 | `marginal_loss_price_da` | 日前边际损耗价 |
| 15 | `da_as_as_mw_synchronized_reserve` | 日前同步备用实际量 |
| 16 | `congestion_price_da` | 日前阻塞价 |
| 17 | `total_pjm_reg_purchases` | 调频购买量 |
| 18 | `total_pjm_rmpcp_cr` | 调频性能信用额 |
