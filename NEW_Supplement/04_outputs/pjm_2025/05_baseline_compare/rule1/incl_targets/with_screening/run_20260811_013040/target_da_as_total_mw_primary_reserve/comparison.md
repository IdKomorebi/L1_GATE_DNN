...
- 全部 38 个字段的普通 DNN：R² = 0.8549（能力上限参照）
...
| 方法 | 说明 | 测试 R² |
|---|---|---|
| Lasso ⭐ | 线性稀疏（1996） | **0.8823** |
| Pearson | 相关系数排序（只看两两关系） | **0.8757** |
| XGBoost | 树模型特征重要性（2016） | **0.8757** |
| LassoNet | 神经网络旁挂线性通路的特征级稀疏（JMLR 2021） | **0.8699** |
| DGatingDNN | 本文方法：可微稀疏门控（NeurIPS 2025） | **0.8660** |
| STG | 随机门控，高斯松弛逼近 L0（ICML 2020） | **0.8516** |

> 同预算下最好的是 **Lasso**（R² = 0.8823）。

## 三、不同字段数下的完整曲线

只比一个预算容易碰巧，下表给出各方法在多个字段数下的表现（对应图 `fig_topk_curve.png`）：

| 方法 | k=1 | k=3 | k=9 | k=18 | k=23 | k=36 | k=38 |
|---|---|---|---|---|---|---|---|
| Lasso | 0.0827 | 0.2380 | 0.6389 | 0.8823 | 0.8796 | 0.8633 | 0.8556 |
| Pearson | 0.4669 | 0.4643 | 0.8742 | 0.8757 | 0.8588 | 0.8608 | 0.8614 |
| XGBoost | 0.4669 | 0.5569 | 0.8643 | 0.8757 | 0.8740 | 0.8623 | 0.8578 |
| LassoNet | 0.4676 | 0.6975 | 0.8615 | 0.8699 | 0.8597 | 0.8661 | 0.8672 |
| DGatingDNN | 0.5798 | 0.8154 | 0.8608 | 0.8660 | 0.8615 | 0.8602 | 0.8493 |
| STG | 0.4549 | 0.5603 | 0.8456 | 0.8516 | 0.8393 | 0.8676 | 0.8628 |
...
| 6 | `prelim_load_avg_hourly` | 预估小时负荷 |
| 7 | `gen_fuel_coal_pct` | 燃煤占比 |
| 8 | `gen_fuel_gas_pct` | 燃气占比 |
| 9 | `total_losses` | 总网损 |
| 10 | `gen_fuel_multiple_fuels_pct` | 多燃料占比 |
| 11 | `net_actual_interchange_mw` | 净实际交换 |
| 12 | `gen_fuel_nuclear_pct` | 核电占比 |
| 13 | `total_pjm_self_sched_reg` | 调频自调度量 |
| 14 | `total_pjm_reg_purchases` | 调频购买量 |
| 15 | `system_energy_price_da` | 日前能量价格 |
| 16 | `total_pjm_rmpcp_cr` | 调频性能信用额 |
| 17 | `gen_fuel_oil_mw` | 燃油出力 |
| 18 | `gen_fuel_wind_pct` | 风电占比 |
