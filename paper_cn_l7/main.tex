...
\end{equation}
其中 $Y\in\mathbb{R}^{n}$ 为待保护的敏感目标字段，$X_i\in\mathbb{R}^{n}$ 为候选字段，$w$ 为候选字段总数，$n$ 为样本数。隐性推断风险分析的目标并非训练高精度预测模型，而是识别一个规模尽可能小的字段集合 $\mathcal{S}$，使其对敏感目标 $Y$ 仍能维持较高的推断精度：
\begin{equation}
...
\subsection{多相关性初筛}
电力数据字段之间的依赖关系具有多样性：既可能因潮流方程而呈现近似线性关系，也可能因市场出清机制、辅助服务约束和分段调度规则而产生复杂的非线性耦合。为避免单一统计指标造成的漏筛风险，本文构建六维相关性向量对每个候选字段 $X_i$ 进行综合评估：
\begin{equation}
R_i=[r_i^{(\metric{NMI})},r_i^{(\metric{Spear})},r_i^{(\metric{Pear})},
r_i^{(\metric{Ken})},r_i^{(\metric{DC})},r_i^{(\metric{HSIC})}].
\end{equation}
式中：六个分量分别对应归一化互信息（NMI）、Spearman 秩相关系数、Pearson 相关系数、Kendall 秩相关系数、距离相关系数（DC）和 Hilbert-Schmidt 独立性准则（HSIC）。这六类指标涵盖了线性相关性度量、秩相关性度量、信息论度量和核方法度量，能够从不同数学角度捕捉字段间的依赖关系。

本文采用"任一指标通过即保留"的宽松初筛策略，即当任意一个指标达到预设阈值时，该字段即被保留进入后续分析：
\begin{equation}
\max_j I\left(r_i^{(j)}\geq \mu_1^{(j)}\right)>0.
\end{equation}
式中：$I(\cdot)$ 为示性函数；$\mu_1^{(j)}$ 为第 $j$ 类指标的初筛阈值。该策略将初筛定位为候选空间的安全压缩而非最终风险判定，确保在任何单一统计证据下表现出足够依赖性的字段均不被遗漏，其最终取舍由后续门控模型中的推断能力和稀疏约束共同决定。

\subsection{L1 门控深度推断模型}
经过多相关性初筛后，得到候选字段集合 $X=\{X_1,\ldots,X_m\}$，其中 $m$ 为通过初筛的字段数。为实现在保持推断精度的同时自动压缩冗余字段，本文在深度神经网络的输入端引入可学习的门控向量 $g=[g_1,\ldots,g_m]$，模型输出为
\begin{equation}
\hat{Y}=f_{\theta}(g\odot X),
\end{equation}
式中：$\odot$ 为逐元素相乘运算；$f_{\theta}$ 为深度神经网络。训练目标函数定义为
\begin{equation}
\mathcal{L}=\frac{1}{n}\sum_{k=1}^{n}(y_k-\hat{y}_k)^2+\lambda\sum_{i=1}^{m}|g_i|.
\end{equation}
式中：第一项为均方误差损失，确保模型对敏感目标的推断精度；第二项为 L1 正则化惩罚，$\lambda$ 为正则化系数，推动门控参数趋向稀疏。在训练过程中，若某字段对降低预测误差具有稳定贡献，则反向传播将维持其较大的门控值 $g_i$；若某字段仅提供冗余或噪声信息，L1 惩罚将持续压低其门控值直至接近零。训练结束后，满足
\begin{equation}
|g_i|\geq \mu_2
\end{equation}
的字段被判定为活跃推断源。式中 $\mu_2$ 为活跃阈值。本文将活跃字段数量记为 $n_s$，并在后续 baseline 对比实验中作为统一字段预算，使不同方法在相同字段数量约束下进行公平比较。

本文所提隐性推断风险分析方法的整体流程如图~\ref{fig:method_flow} 所示。多相关性统计初筛负责广泛覆盖多类型依赖关系，L1 门控深度神经网络在推断训练过程中自动压缩冗余字段并识别活跃推断源，普通深度神经网络再训练则独立验证所选字段集合的实际推断能力。
...
\end{tikzpicture}
\figbicaption{隐性推断风险分析整体流程}{Flowchart of implicit inference risk analysis}
\label{fig:method_flow}
\end{figure}

\subsection{相关性驱动的改进门控}
上述 L1 门控模型中，每个字段的门控参数为独立的可学习变量，无法直接揭示统计相关性证据与门控开闭之间的内在关联。为增强方法的可解释性，本文进一步提出相关性驱动的改进门控模型，将六维相关性向量通过线性映射和 Sigmoid 激活函数生成门控值：
\begin{equation}
g_i=\sigma(W^{\mathrm{T}}R_i+b),
\end{equation}
式中：$W\in\mathbb{R}^{6}$ 为六类相关性指标对应的可学习权重向量，反映各指标对风险判定的相对贡献；$b$ 为全局风险容忍偏置；$\sigma(\cdot)$ 为 Sigmoid 激活函数。由 Sigmoid 函数的性质可知，当 $W^{\mathrm{T}}R_i+b=0$ 时，$g_i=0.5$。因此，$g_i=0.5$ 构成门控开闭的自然判定边界，对应如下风险边界方程：
\begin{equation}
W^{\mathrm{T}}R_i+b=0.
\end{equation}
当 $g_i>0.5$ 时，该字段的相关性证据足以使门控打开，字段被判定为潜在推断源；当 $g_i<0.5$ 时，该字段被视为冗余或低风险字段，门控趋于关闭。与普通 L1 门控模型相比，改进门控的核心价值并非提升推断精度 $R^2$，而在于将"六类统计指标如何共同决定字段风险"这一问题转化为可学习的统一边界，从而取代人工对每个指标独立设置阈值的方式，为风险判定提供更具解释性的定量依据。
