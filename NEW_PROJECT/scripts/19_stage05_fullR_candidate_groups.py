...
def _load_stage05_data(stage04_interface: Path, cfg: dict[str, Any], args: argparse.Namespace) -> Stage05Data:
    ...
    anchor_features = list(stage04["active_candidate_features"])
    if int(args.max_anchors) > 0:
        anchor_features = anchor_features[: int(args.max_anchors)]
    candidate_features = list(stage04["residual_candidate_features"])
    missing_anchors = [feature for feature in anchor_features if feature not in candidate_features]
    if missing_anchors:
        raise ValueError(f"Anchor features are not in full R candidate pool: {missing_anchors}")
    all_needed = list(dict.fromkeys([*ci_features, *candidate_features]))
    ...
    return Stage05Data(
        ...
        X_candidate_raw=aligned[candidate_features].to_numpy(dtype=np.float32),
        ...
        ci_r2=float(pd.read_csv(stage02["repruned_replaceability_summary_csv"]).set_index("target_feature").loc[xi, "C_i_r2"]),
        ci_plus_r_r2=float(pd.read_csv(stage02["repruned_replaceability_summary_csv"]).set_index("target_feature").loc[xi, "C_i_plus_R_r2"]),
        full_all_features_r2=float(pd.read_csv(stage02["repruned_replaceability_summary_csv"]).set_index("target_feature").loc[xi, "full_all_features_r2"]),
        train_idx=train_idx,
        val_idx=val_idx,
...
def _make_training_arrays(data: Stage05Data) -> tuple[DataLoader, tuple[np.ndarray, np.ndarray, np.ndarray], dict[str, Any]]:
    ...
    X_a_all, a_mean, a_std = _standardize(data.X_candidate_raw[data.train_idx], data.X_candidate_raw)
...
def _train_trial(
    ...
    model = AnchoredMultiHeadDGatedResidualNet(
        ...
        active_dim=len(data.candidate_features),
        hidden_dims=args.hidden_dims,
        dgate_depth=int(args.dgate_depth),
        anchor_indices=[data.candidate_features.index(anchor) for anchor in data.anchor_features],
    ).to(device)
...
def _extract_heads(
    *,
    ...
    rows = []
    for h, anchor in enumerate(data.anchor_features):
        anchor_idx = data.candidate_features.index(anchor)
        non_anchor_norms = np.delete(norms[h], anchor_idx)
        base = float(np.max(non_anchor_norms)) if len(non_anchor_norms) else 0.0
        selected = [anchor]
        for j, feature in enumerate(data.candidate_features):
            if j == anchor_idx:
                continue
            if base > 0 and float(norms[h, j]) >= threshold_ratio * base:
                selected.append(feature)
        keep_full = np.ones((len(data.anchor_features), len(data.candidate_features)), dtype=np.float32)
        keep_anchor_off = keep_full.copy()
        keep_anchor_off[h, anchor_idx] = 0.0
...
def _write_report(run_dir: Path, data: Stage05Data, best: dict[str, Any], validation_df: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        f"- xi: `{data.xi}`",
        f"- anchor/head count: `{len(data.anchor_features)}`",
        f"- full-R candidate count: `{len(data.candidate_features)}`",
...
def main() -> None:
    ...
    parser.add_argument("--stage-dir", default="stage05_candidate_groups_fullR")
    parser.add_argument("--run-name", default=None)
    parser.add_argument("--max-anchors", type=int, default=0, help="Use only the first N active anchors for a small probe.")
    parser.add_argument("--target-r2", type=float, default=None, help="Override dynamic target. Defaults to max(min-target-r2, C_i+R-target-margin).")
    parser.add_argument("--min-target-r2", type=float, default=0.95)
    ...
    parser.add_argument("--lambda-sweep", nargs="+", type=float, default=[3e-5, 1e-4, 3e-4, 1e-3])
    parser.add_argument("--threshold-sweep", nargs="+", type=float, default=[0.35, 0.50, 0.65, 0.80])
    ...
    parser.add_argument("--max-validate-groups", type=int, default=64)
    args = parser.parse_args()

    cfg = load_config(args.config)
    data = _load_stage05_data(Path(args.stage04_interface), cfg, args)
    dynamic_target = max(float(args.min_target_r2), float(data.ci_plus_r_r2) - float(args.target_margin))
    args.target_r2 = float(args.target_r2) if args.target_r2 is not None else dynamic_target
    ...
    save_json(
        data_dir / "data_summary.json",
        {
            ...
            "anchor_features": data.anchor_features,
            "candidate_features": data.candidate_features,
            "anchor_count": len(data.anchor_features),
    ...
    save_json(
        ...
        {
            "stage": "stage05_candidate_groups_fullR",
            "stage04_interface": str(Path(args.stage04_interface).resolve()),
            "xi": data.xi,
            "target_r2": args.target_r2,
            "target_rule": "max(min_target_r2, C_i_plus_R_r2 - target_margin)",
            "min_target_r2": args.min_target_r2,
            "target_margin": args.target_margin,
            "ci_r2": data.ci_r2,
            "ci_plus_r_r2": data.ci_plus_r_r2,
    ...
    probe_candidates = all_groups.drop_duplicates("global_group_key").head(int(args.max_validate_groups)).copy()
    ...
    save_json(
        run_dir / "stage05_candidate_group_interface.json",
        {
            ...
            "ci_features": data.ci_features,
            "anchor_features": data.anchor_features,
            "candidate_pool_features": data.candidate_features,
            "candidate_pool": "full_R_from_stage04",
